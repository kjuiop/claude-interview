...
{위 개념을 실제 면접에서 600자 이상 분량의 완성된 줄글로 작성.
 개략식 나열 ❌ → 연속된 문장 ✅
...
- 난이도 순서로 배치: 기초 → 중급 → 심화
- **현실성 필터**: 질문 추가 전 "한국 백엔드 면접관이 실제로 물어볼 법한가?" 자문
  - 탈락: 런타임/컴파일러 내부 구현, 어셈블리 수준, 학술 논문 수준 세부 사항
  - 통과: 개념 정의, 동작 원리(블랙박스 수준), 트레이드오프, 실무 패턴, 장애 대응 경험
  - 심화 질문도 "내부 구현 암기"가 아닌 "실무 판단력과 트레이드오프 이해" 수준이어야 함
