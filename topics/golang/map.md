...
**모범 답변 방향**:
- 값 타입(`int`, `bool`, `string`, `array`, `struct`)은 할당·전달 시 값 전체가 복사
- 참조 타입(`map`, `slice`, `channel`, `pointer`)은 내부적으로 포인터를 갖고 있어 같은 힙 데이터를 공유
- map이 참조 타입인 이유: `make(map[K]V)` 시 런타임이 힙에 `hmap` 구조체를 할당하고, map 변수는 그 `*hmap` 포인터를 갖기 때문
...
- `var m map[K]V`는 nil map — hmap이 할당되지 않은 상태 (zero value)
- `make(map[K]V)` 또는 `map[K]V{}`는 empty map — 빈 hmap이 할당된 상태
- nil map 읽기: 안전. zero value 반환
- nil map 쓰기: **런타임 panic** — `assignment to entry in nil map`
- 흔한 실수: struct 필드로 map을 선언만 하고 초기화 안 한 채 쓰기 시도

---

**Q. Go map의 순회 순서가 비결정적인 이유는?**

- Go 런타임이 `range` 순회 시작 위치를 **의도적으로 무작위화**
- 이유: 개발자가 순서에 의존하는 코드를 원천 차단, 버킷 구조상 삽입 순서 보장 불가
- 정렬 순회가 필요하면 키를 slice로 추출 → `sort.Strings` → 순회

---

**Q. 여러 goroutine이 동시에 map을 읽고 쓰면 어떤 문제가 발생하고, 어떻게 해결하나요?**

- Go map은 thread-safe하지 않음. 동시 읽기/쓰기 시 런타임 패닉 또는 데이터 손상
- 해결책: `sync.RWMutex` (범용), `sync.Map` (읽기 heavy), Sharded Map (쓰기 heavy)
- `sync.Map` 주의: 쓰기가 빈번하면 dirty 맵 승격 비용으로 `RWMutex` 보다 느릴 수 있음
