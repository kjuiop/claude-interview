...
카테노이드 채팅 서버 개발 당시 메시지 전송, 읽음 처리, 채팅방 목록 조회 등의 API를 설계할 때 이 원칙을 기반으로 URI 리뷰를 진행했습니다. `/api/v1/rooms/{roomId}/messages` 처럼 계층적 자원 구조를 URI로 표현하고, 전체 교체가 필요한 경우와 특정 필드만 수정하는 경우를 PUT/PATCH로 명확히 구분했습니다. REST 설계에서 흔히 발생하는 실수 중 하나는 GET 요청에 body를 담거나 POST로 조회를 처리하는 것입니다. 또한 에러 응답에 구조화된 JSON 바디를 항상 함께 반환해 클라이언트가 에러 원인을 코드 레벨에서 파악할 수 있게 하는 것도 중요합니다. `{"error": "INVALID_EMAIL", "message": "이메일 형식이 잘못됐습니다"}` 형태의 에러 바디가 없으면 상태 코드만으로는 어떤 필드가 잘못됐는지, 어떤 권한이 부족한지 알 수 없기 때문입니다.
